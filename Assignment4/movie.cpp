...
using namespace std;
	...
	movie::movie(int id, string name, string date)
		:movieName(name), releaseDateMovie(date), longmovieID(id)
	{
		string line;
	...
	string movie::getContent()
	{
		...
		string movieIDD = to_string(longmovieID);
		string temp = "";
		size_t pos;
	...
	std::vector<long> movie::fullMovieList()
	{
		regex pattern("[^\\s]+");
		smatch match;
